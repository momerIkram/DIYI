...
# Initialize session state for customer detail view and action view
...
if choice == "Dashboard":
    ...
    try:
        projects = db.get_all_projects() # This might fail if Projects.CustomerID is missing
        ...
        projects = [] # Ensure projects is an empty list to prevent further errors
    ...
    col1.metric("Total Customers", len(customers) if customers else 0)
    col2.metric("Total Products", len(products) if products else 0)
    col3.metric("Total Suppliers", len(suppliers) if suppliers else 0)
    col4.metric("Total Materials", len(materials) if materials else 0)
    ...
    if invoices:
        ...
        if not recent_invoices_df.empty:
            ...
            if 'PaymentDate_Display' in recent_invoices_df.columns:
                cols_recent_inv_display.append('PaymentDate_Display')
            
            # Filter for existing columns only, in case some are missing from db query
            final_cols_recent_inv = [col for col in cols_recent_inv_display if col in recent_invoices_df.columns]
            st.dataframe(recent_invoices_df[final_cols_recent_inv], use_container_width=True)
        else:
    ...
    if st.session_state.customer_management_action_view == "List":
        ...
        if action == "View All Customers":
            ...
            if not customers_list_del_rows: 
                ...
                selected_cust_disp_del = st.selectbox("Select Customer to Delete", list(customer_options_del.keys()), key="del_cust_select_list_view")
    ...
    if action_sup == "View All":
        ...
        if suppliers_list_rows:
            st.dataframe(db.rows_to_dicts(suppliers_list_rows), use_container_width=True)
        else:
        ...
        if not suppliers_list_edit_rows:
            ...
            if selected_sup_disp:
                ...
                sup_data_row = db.get_supplier_by_id(sup_id_to_edit)
                sup_data = dict(sup_data_row[0]) if isinstance(sup_data_row, list) and sup_data_row else dict(sup_data_row) if sup_data_row else None
                if sup_data:
...
# Ensure similar db.rows_to_dicts() usage where lists of rows are expected and then processed.
# For single item fetches like get_product_by_id, ensure it returns a dict or is converted.

# Example for Product Management (Edit Product, assuming get_product_by_id returns one row/dict)
# prod_data_edit_main_row = db.get_product_by_id(prod_id_to_edit_main)
# prod_data_edit_main = dict(prod_data_edit_main_row[0]) if isinstance(prod_data_edit_main_row, list) and prod_data_edit_main_row else dict(prod_data_edit_main_row) if prod_data_edit_main_row else None

# Make sure all other sections follow this pattern for data fetching and processing,
# especially where db functions might return raw sqlite3.Row objects or lists of them.
# The db.rows_to_dicts() helper is key if functions return lists of rows.
# For functions returning a single row, ensure they either return a dict or you convert it.

# (Keep existing Material Management, Product Management, etc. sections)
# The provided code only shows up to Supplier Management, so I'll assume other sections
# will be reviewed for similar patterns if they interact with database rows directly.
# The key changes are in Dashboard (date display) and Customer Management (major refactor).

# --- (Continuing with the rest of your original code from Supplier Services onwards) ---
# --- Make sure to adapt data fetching for single items in Edit forms if needed, like shown for Edit Supplier ---
# --- e.g. some_data_row = db.get_X_by_id(id); some_data = dict(some_data_row[0]) if ... else dict(some_data_row) ... ---
    ...
    if action_ss == "View All Services":
        ...
        if services_list:
            ...
            for service_row_dict in services_list: # Already dicts from db.rows_to_dicts
                ...
                if not ss_supplier_id or not ss_service_name or not ss_service_type_final or ss_cost < 0: # Check final type
                    ...
                    try:
                        new_service_id, expense_auto_logged = db.add_supplier_service(
                            ...
                            None, # Receipt path initially None
                            ss_description
                        )
                        ...
                        if new_service_id and ss_receipt_upload:
                            ...
                            if receipt_file_path:
                                # Fetch original IsExpenseLogged if needed, or assume db.update_supplier_service handles it or is not relevant to update here
                                db.update_supplier_service(new_service_id, ss_supplier_id, ss_project_id,
                                                           ss_service_name, ss_service_type_final,
                                                           ss_service_date.strftime("%Y-%m-%d"), ss_cost,
                                                           receipt_file_path, ss_description, is_expense_logged=expense_auto_logged) # Pass IsExpenseLogged
                        ...
                        elif new_service_id and ss_cost > 0: # If new_service_id is true, implies success to some extent
        ...
        if not services_list_edit: st.info("No services to edit.")
            ...
            service_options = {f"{s['ServiceName']} by {s.get('SupplierName', 'Unknown Supplier')} (ID: {s['ServiceID']})": s['ServiceID'] for s in services_list_edit} # Use .get for SupplierName
            ...
            if selected_service_disp:
                ...
                service_data_row = db.get_supplier_service_by_id(service_id_to_edit)
                service_data = dict(service_data_row[0]) if isinstance(service_data_row, list) and service_data_row else dict(service_data_row) if service_data_row else None
                ...
                if service_data:
                    with st.form(f"edit_supplier_service_form_{service_id_to_edit}"):
                        ...
                        except StopIteration: supplier_index = 0 
                        ...
                        current_project_id_ss = service_data.get('ProjectID') # Use .get for safety
                        ...
                        ss_is_expense_logged_edit = service_data.get('IsExpenseLogged', False) # Get current state
        ...
        if not services_list_del: st.info("No services to delete.")
            ...
            service_options_del = {f"{s['ServiceName']} by {s.get('SupplierName','Unknown Supplier')} (ID: {s['ServiceID']})": s['ServiceID'] for s in services_list_del}
...
# ... (Assuming the rest of the file follows)
    ...
    if action_mat == "View All":
        ...
        materials_list_rows = db.get_all_materials(search_term=search_term_mat)
        if materials_list_rows: 
            st.dataframe(db.rows_to_dicts(materials_list_rows), use_container_width=True)
        else: 
        ...
        with st.form("add_material_form_page", clear_on_submit=True):
            ...
            m_qty = st.number_input("Quantity In Stock", format="%.2f", min_value=0.0) # ensure non-negative
            selected_sup_name_mat = st.selectbox("Supplier", list(supplier_map_mat.keys()), key="add_mat_supplier_page") 
        ...
        if not materials_list_edit_rows: 
            ...
            selected_mat_disp = st.selectbox("Select Material", list(material_options.keys()), key="edit_mat_select_page")
            if selected_mat_disp:
                mat_id_to_edit = material_options[selected_mat_disp]
                mat_data_row = db.get_material_by_id(mat_id_to_edit)
                mat_data = dict(mat_data_row[0]) if isinstance(mat_data_row, list) and mat_data_row else dict(mat_data_row) if mat_data_row else None
                if mat_data:
                    with st.form(f"edit_material_form_page_{mat_id_to_edit}"):
                        m_name = st.text_input("Material Name*", value=mat_data['MaterialName'])
                        m_type = st.text_input("Material Type", value=mat_data.get('MaterialType',''))
                        ...
                        try: 
                            ...
                            mat_supplier_index = 0 # Default to "None (No Supplier)"
                        selected_sup_name_mat_edit = st.selectbox("Supplier", material_supplier_keys_list, index=mat_supplier_index, key="edit_mat_supplier_page") 
        ...
        if not materials_list_del_rows: 
            ...
            selected_mat_disp_del = st.selectbox("Select Material to Delete", list(material_options_del.keys()), key="del_mat_select_page") 
    ...
    if action_prod == "View All":
        ...
        products_list_rows = db.get_all_products(search_term=search_term_prod)
        if products_list_rows:
            products_list = db.rows_to_dicts(products_list_rows)
            df_products = pd.DataFrame(products_list)
            cols_to_display_prod = [col for col in df_products.columns if col not in ['ImagePath','SupplierID']] # Start with most
            if 'SupplierName' in df_products.columns and 'SupplierID' in cols_to_display_prod : 
                pass # SupplierName is preferred if available from join in db.get_all_products
            elif 'SupplierID' in df_products.columns and 'SupplierName' not in df_products.columns:
                # If only SupplierID is there, you might want to show it or map it.
                # For now, let's assume db.get_all_products provides SupplierName if joined.
                pass
            
            # Ensure essential columns are present
            final_cols_prod_view = []
            default_prod_cols = ['ProductID', 'ProductName', 'SKU', 'Category', 'SellingPrice', 'QuantityInStock', 'SupplierName'] # Ideal
            for col in default_prod_cols:
                if col in df_products.columns:
                    final_cols_prod_view.append(col)
            # Add any other existing columns not in default but desired
            for col in df_products.columns:
                if col not in final_cols_prod_view and col not in ['ImagePath', 'SupplierID']:
                     final_cols_prod_view.append(col)

            st.dataframe(df_products[final_cols_prod_view], use_container_width=True)

            for prod_row_dict_main in products_list: # Iterate over list of dicts
        ...
        with st.form("add_product_form_main", clear_on_submit=False): # clear_on_submit can be True if rerun is desired
            p_name = st.text_input("Product Name*")
            p_sku = st.text_input("SKU (Unique)")
            p_desc = st.text_area("Description")
            p_cat = st.text_input("Category (e.g., Sofa, Table)")
            p_mat_type_prod = st.text_input("Material Type (e.g., Wood, Fabric)") # Renamed for clarity
            ...
            if submitted_add_prod:
                if not p_name or not p_sku: 
                    ...
                    # Check if product with SKU already exists before trying to save image with SKU
                    # This check should ideally be in db.add_product or a separate db utility
                    # For now, assume unique SKUs are enforced by DB or handled in db.add_product
                    if p_uploaded_image_add: 
                        p_image_path_add = save_uploaded_product_image(p_uploaded_image_add, p_sku) # Using SKU for filename part
                    try:
                        ...
                        if p_image_path_add and os.path.exists(p_image_path_add): 
                            try:
                                os.remove(p_image_path_add) # Clean up image if DB add fails
                            except OSError as e_os_remove:
                                st.warning(f"Failed to remove uploaded image after error: {e_os_remove}")
        ...
        if not products_list_edit_rows: 
            ...
            if selected_prod_disp_edit_main:
                ...
                prod_data_row = db.get_product_by_id(prod_id_to_edit_main)
                prod_data_edit_main = dict(prod_data_row[0]) if isinstance(prod_data_row, list) and prod_data_row else dict(prod_data_row) if prod_data_row else None
                ...
                if prod_data_edit_main:
                    with st.form(f"edit_product_form_main_page_{prod_id_to_edit_main}"):
                        ...
                        # SKU might be critical for image naming, consider if it should be editable
                        # If SKU is changed, image path might need update or old image might be orphaned.
                        ...
                        if updated_prod:
                            if not p_name_edit or not p_sku_edit: 
                                ...
                                if p_new_uploaded_image_edit:
                                    # Determine filename based on product ID or NEW SKU. Product ID is safer if SKU can change.
                                    # save_uploaded_product_image uses product_id_or_sku.
                                    # If SKU changed, new image will have new SKU in name. Old image might be orphaned if not removed.
                                    new_image_filename_base = p_sku_edit # Or prod_id_to_edit_main for stability
                                    
                                    # Attempt to remove old image if it exists
                                    if current_image_path_edit and os.path.exists(current_image_path_edit):
                                        # Only remove if the new image is different or if SKU changed and filename depends on SKU
    ...
    customer_map_proj = {"None (No Customer Associated)": None} # Allow projects not tied to a customer initially if design permits
    ...
    if action_proj == "View All":
        ...
        if projects_list_rows:
            ...
            # Assuming db.get_all_projects joins with Customers for CustomerName
            ...
            st.info("No projects found. This could also be due to the 'Projects.CustomerID' issue if not fixed in db.py.")
        ...
        with st.form("add_project_form_main_key", clear_on_submit=True):
            ...
            selected_cust_name_proj = st.selectbox("Customer*", list(customer_map_proj.keys()), index=0, key="add_proj_customer_main_key") # Ensure default is "None" if desired
            ...
            if submitted_proj:
                ...
                elif not pr_customer_id: # Make customer mandatory for projects
        ...
        if not projects_list_edit_rows: 
            ...
            if selected_proj_disp_edit_main_page:
                ...
                proj_data_row = db.get_project_by_id(proj_id_to_edit_main_page)
                proj_data_main_page = dict(proj_data_row[0]) if isinstance(proj_data_row, list) and proj_data_row else dict(proj_data_row) if proj_data_row else None
                ...
                if proj_data_main_page:
                    with st.form(f"edit_project_form_main_page_{proj_id_to_edit_main_page}"):
                        ...
                        try: 
                            ...
                            proj_customer_index = 0 # Default to "None"
                        ...
                        pr_status_edit = st.selectbox("Status", project_status_options, 
                                                      index=project_status_options.index(proj_data_main_page['Status']) if proj_data_main_page.get('Status') in project_status_options else 0)
                        ...
                        if updated_proj_edit:
                            if not pr_name_edit: 
                                st.error("Project Name required.")
                            elif not pr_customer_id_edit: # Enforce customer for project
                                st.error("Customer is required for a project.")
                            else:
                                try:
    ...
    if action_order_main == "View All Orders":
        ...
        if all_db_orders_rows:
            ...
            # Assuming db.get_all_orders joins for CustomerName, ProjectName
            ...
            order_ids_for_view_main = [o['OrderID'] for o in all_db_orders_main] # Already dicts
            if order_ids_for_view_main:
                ...
                if selected_order_id_view_main:
                    ...
                    if items_for_order_rows:
                        ...
                        st.dataframe(db.rows_to_dicts(items_for_order_rows), use_container_width=True)
                    else: 
        ...
        with st.form("add_order_form_db_main_key", clear_on_submit=False):
            ...
            if o_customer_id_main:
                ...
                cust_details_for_addr_main = dict(cust_details_row[0]) if isinstance(cust_details_row, list) and cust_details_row else dict(cust_details_row) if cust_details_row else None
            ...
            # Use a separate session state key for the text_area if clear_on_submit=False, or ensure it's re-evaluated
            o_shipping_address_main = st.text_area("Shipping Address", value=initial_shipping_address_main, key="order_ship_addr_main_input_key_create")
            o_notes_main = st.text_area("Order Notes")
            
            st.markdown("---"); st.subheader("Order Items")
            if 'current_order_items_main' not in st.session_state: 
            ...
            if prod_id_for_price_main:
                ...
                prod_details_main = dict(prod_details_row[0]) if isinstance(prod_details_row, list) and prod_details_row else dict(prod_details_row) if prod_details_row else None
            ...
            if item_cols_main[4].button("➕ Add", key="order_add_item_button_key_main"):
                if prod_id_for_price_main and qty_item_main > 0:
                    # Re-fetch prod_details_main for current stock check inside button press
                    prod_details_add_row = db.get_product_by_id(prod_id_for_price_main)
                    prod_details_add_main = dict(prod_details_add_row[0]) if isinstance(prod_details_add_row, list) and prod_details_add_row else dict(prod_details_add_row) if prod_details_add_row else None

                    if prod_details_add_main and prod_details_add_main.get('QuantityInStock', 0) < qty_item_main: 
                        st.warning(f"Not enough stock for {prod_details_add_main['ProductName']} (Available: {prod_details_add_main.get('QuantityInStock',0)}).")
                    elif prod_details_add_main:
                        st.session_state.current_order_items_main.append({
                            'ProductID': prod_id_for_price_main, 
                            'ProductName': prod_details_add_main['ProductName'], 
                    ...
                    st.warning("Please select a product and quantity.")
            ...
            if submitted_order_db_main:
                if not o_customer_id_main: 
                    ...
                    # Fetch the current value from the widget state for text_area
                    final_shipping_address_main_val = st.session_state.get("order_ship_addr_main_input_key_create", initial_shipping_address_main)
                    try:
                        new_order_id_main = db.add_order(
                            o_order_date_main.strftime("%Y-%m-%d"), o_customer_id_main, o_project_id_main,
                            o_order_status_main, 0, o_payment_status_main, 
                        ...
                        for item_data_main in st.session_state.current_order_items_main:
                            ...
                            db.update_product_stock(item_data_main['ProductID'], -item_data_main['QuantitySold']) # Negative to decrease stock
        ...
        if not all_orders_edit_rows:
            ...
            if selected_order_key_edit:
                ...
                order_data = dict(order_data_row[0]) if isinstance(order_data_row, list) and order_data_row else dict(order_data_row) if order_data_row else None
                ...
                if order_data:
                    ...
                    with st.form(f"edit_order_form_{order_id_to_edit}"):
                        ...
                        try:
                            order_status_idx = order_status_options_edit.index(order_data['OrderStatus'])
                        except ValueError: order_status_idx = 0
                        o_order_status_edit = col_ord_stat1_edit.selectbox("Order Status", order_status_options_edit, index=order_status_idx)
                        
                        try:
                            payment_status_idx = payment_status_options_edit.index(order_data['PaymentStatus'])
                        except ValueError: payment_status_idx = 0
    ...
    # Ensure CustomerName is available from get_all_projects if displaying it
    ...
    if action_inv == "View All Invoices":
        ...
        if not invoices_list_edit_rows:
            ...
            if selected_invoice_key_edit:
                ...
                invoice_data = dict(invoice_data_row[0]) if isinstance(invoice_data_row, list) and invoice_data_row else dict(invoice_data_row) if invoice_data_row else None
                ...
                if invoice_data:
                    with st.form(f"edit_invoice_form_{invoice_id_to_edit}"):
                        ...
                        if invoice_data.get('ProjectID'):
                            ...
                            project_details_for_edit = dict(project_details_row[0]) if isinstance(project_details_row, list) and project_details_row else dict(project_details_row) if project_details_row else None
                        ...
                        if invoice_data.get('CustomerID'):
                            ...
                            customer_details_for_edit = dict(customer_details_row[0]) if isinstance(customer_details_row, list) and customer_details_row else dict(customer_details_row) if customer_details_row else None
    ...
    if action_exp_main_page == "View All":
        ...
        if all_db_expenses_rows:
            ...
            if 'Amount' in df_expenses_main_page.columns: 
                df_expenses_main_page['Amount_Display'] = df_expenses_main_page['Amount'].apply(lambda x: f"Rs. {x:,.2f}")
            
            # Assuming db.get_all_expenses joins for ProjectName and potentially SupplierServiceName
        ...
        with st.form("add_manual_expense_form_db_main_key", clear_on_submit=True):
            ...
            if submitted_exp_man_main:
                if not desc_exp_man_main or amt_exp_man_main <= 0: 
                    ...
                    try:
                        db.add_expense(exp_date_man_main.strftime("%Y-%m-%d"), desc_exp_man_main, cat_exp_man_main, amt_exp_man_main, vendor_exp_man_main, exp_project_id_man_main, receipt_ref_exp_man_main, None) # SupplierServiceID is None
    ...
    if report_type_main == "Overall Financial Summary":
        ...
        if projects_rep_rows:
            ...
            for proj in projects_rep:
                ...
                expenses_for_proj_rows = db.get_expenses_by_project_id(proj_id) 
                expenses_for_proj = db.rows_to_dicts(expenses_for_proj_rows) if expenses_for_proj_rows else []
                cost_for_proj = sum(exp['Amount'] for exp in expenses_for_proj if exp.get('Amount'))
            ...
            st.info("No projects available for reporting. This could also be due to the 'Projects.CustomerID' issue if not fixed in db.py.")
