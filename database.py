...
RECEIPT_DIR = os.path.join(DATABASE_DIR, "receipts")
...
def init_db():
    ...
    ''')
    _add_column_if_not_exists(cursor, "materials", "Category", "TEXT")
...
def add_customer(name, email, phone, reference_id_input, bill_addr, ship_addr, notes):
    ...
    try:
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (name, email, phone, bill_addr, ship_addr or bill_addr, reg_date, notes, None))
...
def update_customer(cust_id, name, email, phone, reference_id_input, bill_addr, ship_addr, notes):
    ...
    try:
        ...
        final_reference_id = current_customer_row['ReferenceID']

        if reference_id_input and reference_id_input.strip():
            if reference_id_input.strip() != current_customer_row['ReferenceID']:
                final_reference_id = reference_id_input.strip()
        elif not current_customer_row['ReferenceID']:
...
def add_material(name, category, subtype, unit, cost_unit, qty, supplier_id):
    conn = get_db_connection()
    cursor = conn.cursor()
...
def update_material(mat_id, name, category, subtype, unit, cost_unit, qty, supplier_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    last_update = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    try:
...
def remove_material_from_project(project_material_id):
    ...
    try:
        ...
        if material_to_return:
            update_material_stock(material_to_return['MaterialID'], material_to_return['QuantityUsed'])
    ...
    try:
        ...
        if cost > 0 and service_pk_id:
            ...
            new_expense_pk_id = add_expense(
                ...
                supplier_service_id=service_pk_id,
...
def update_supplier_service_receipt_path(service_id, receipt_path):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
...
def get_services_for_project(project_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
...
def add_expense(exp_date, desc, category, amount, vendor, project_id, receipt_ref, supplier_service_id=None, is_internal_call=False):
    conn = get_db_connection()
    cursor = conn.cursor()
    expense_pk_id = None
    try:
...
def get_expenses_by_project_id(project_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM expenses WHERE ProjectID = ?", (project_id,))
    expenses = cursor.fetchall()
    conn.close()
    return expenses
# --- END NEW FUNCTION ---
